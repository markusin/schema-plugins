...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="iso19139-2CompleteTab">
		...
		<!-- Debug start <xsl:message> Template gmiCompleteTab </xsl:message> <xsl:message>### TabLink <xsl:value-of 
		select="$tabLink"/> ### </xsl:message> <xsl:message>### Schema <xsl:value-of select="$schema"/> ### </xsl:message> 
		
		<xsl:message>### Current tab is: <xsl:value-of select="$currTab" /> ###</xsl:message> Debug start -->
		
		<!-- call iso19139CompleteTab located in iso19139 -->
	...
	<xsl:template mode="iso19139-2"
		match="gmi:EURAC_LegalConstraints|gmi:EURAC_SecurityConstraints">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<!-- Extra tabs for ISO Profil for GMI -->
	<xsl:template mode="extraTab" match="/">
		...
		<!-- Debug start -
		<xsl:message>
		Template extraTab mode
		</xsl:message>
		<xsl:message>
		### TabLink
		<xsl:value-of select="$tabLink" />
		###
		</xsl:message>
		<xsl:message>
		### Schema
		...
		<!-- <xsl:call-template name="displayTab"> <xsl:with-param name="tab" select="'gmiExtraElement'" /> 
		<xsl:with-param name="text" select="/root/gui/strings/gmiExtraMD" /> <xsl:with-param name="indent" select="'&#xA0;&#xA0;&#xA0;'" 
		
		/> <xsl:with-param name="tabLink" select="$tabLink" /> </xsl:call-template> -->
		<!-- The next template will create additional tabs on the left side -->
		<xsl:if test="$schema='iso19139-2'">
			<!-- GMI tabs -->
			<xsl:if test="/root/gui/config/metadata-tab/gmi">
				<xsl:call-template name="displayTab">
					<xsl:with-param name="tab" select="'packages'"/>
					<xsl:with-param name="text"
						select="/root/gui/strings/gmiTab"/>
					<xsl:with-param name="tabLink" select="''"/>
				</xsl:call-template>
				
				<xsl:call-template name="displayTab">
					<xsl:with-param name="tab" select="'gmiTabDesc'"/>
					<xsl:with-param name="text"
						select="/root/gui/strings/gmiTabDesc"/>
					<xsl:with-param name="indent" select="'&#xA0;&#xA0;&#xA0;'"/>
					<xsl:with-param name="tabLink" select="$tabLink"/>
				</xsl:call-template>
				
				<xsl:call-template name="displayTab">
					<xsl:with-param name="tab" select="'gmiTabTech'"/>
					<xsl:with-param name="text"
						select="/root/gui/strings/gmiTabTech"/>
					<xsl:with-param name="indent" select="'&#xA0;&#xA0;&#xA0;'"/>
					<xsl:with-param name="tabLink" select="$tabLink"/>
				</xsl:call-template>
				
				<xsl:call-template name="displayTab">
					<xsl:with-param name="tab" select="'gmiTabQua'"/>
					<xsl:with-param name="text"
						select="/root/gui/strings/gmiTabQua"/>
					<xsl:with-param name="indent" select="'&#xA0;&#xA0;&#xA0;'"/>
					<xsl:with-param name="tabLink" select="$tabLink"/>
				</xsl:call-template>
				
				<xsl:call-template name="displayTab">
					<xsl:with-param name="tab" select="'gmiTabAcc'"/>
					<xsl:with-param name="text"
						select="/root/gui/strings/gmiTabAcc"/>
					<xsl:with-param name="indent" select="'&#xA0;&#xA0;&#xA0;'"/>
					<xsl:with-param name="tabLink" select="$tabLink"/>
				</xsl:call-template>
				
				<xsl:call-template name="displayTab">
					<xsl:with-param name="tab" select="'gmiTabMd'"/>
					<xsl:with-param name="text"
						select="/root/gui/strings/gmiTabMd"/>
					<xsl:with-param name="indent" select="'&#xA0;&#xA0;&#xA0;'"/>
					<xsl:with-param name="tabLink" select="$tabLink"/>
				</xsl:call-template>
			</xsl:if>
	...
	<xsl:template mode="iso19139"
		match="gmi:MI_Metadata|*[@gco:isoType='gmi:MI_Metadata']" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		<xsl:param name="embedded"/>
	...
	<xsl:template name="gmiTabDesc">
		...
		<xsl:call-template name="complexElementGuiWrapper">
			<xsl:with-param name="title" select="/root/gui/strings/gmiTabDesc"/>
			<xsl:with-param name="id"
				select="generate-id(/root/gui/strings/gmiTabDesc)"/>
			<xsl:with-param name="content">
				...
				<xsl:apply-templates mode="complexElement"
					select="gmd:identificationInfo/*/srv:operatesOn[1]">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit" select="true()"/>
					<xsl:with-param name="content">
						<xsl:apply-templates mode="elementEP"
							select="gmd:identificationInfo/*/srv:operatesOn|
				...
				<xsl:if
					test="not(gmd:identificationInfo/*/gmd:descriptiveKeywords)">
					<xsl:apply-templates mode="elementEP"
						select="
				...
				<xsl:for-each select="gmd:identificationInfo/*/gmd:extent">
					<xsl:apply-templates mode="complexElement" select=".">
						...
						<xsl:with-param name="content">
							...
							<xsl:apply-templates mode="elementEP"
								select="./gmd:EX_Extent/geonet:child[string(@name)='geographicElement']">
								<xsl:with-param name="schema" select="$schema"/>
								<xsl:with-param name="edit" select="$edit"/>
								<xsl:with-param name="force" select="true()"/>
							</xsl:apply-templates>
					...
					<xsl:apply-templates mode="elementEP"
						select="gmd:EX_Extent/gmd:verticalElement|gmd:EX_Extent/gmd:temporalElement">
						<xsl:with-param name="schema" select="$schema"/>
						<xsl:with-param name="edit" select="$edit"/>
					</xsl:apply-templates>
				</xsl:for-each>
	...
	<xsl:template name="gmiTabTech">
		...
		<xsl:call-template name="complexElementGuiWrapper">
			<xsl:with-param name="title" select="/root/gui/strings/gmiTabTech"/>
			<xsl:with-param name="id"
				select="generate-id(/root/gui/strings/gmiTabTech)"/>
			<xsl:with-param name="content">
				...
				<xsl:apply-templates mode="elementEP"
					select="		
		            gmd:identificationInfo/*/gmd:CI_Citation|
					gmd:identificationInfo/*/geonet:child[string(@name)='CI_Citation']
					">
					<!-- gmd:identificationInfo/*/gmi:relatedCitation/gmd:CI_Citation|
					
					gmd:identificationInfo/*/gmi:relatedCitation/geonet:child[string(@name)='CI_Citation']">-->
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit" select="$edit"/>
				</xsl:apply-templates>
	...
	<xsl:template name="gmiTabQua">
		...
		<xsl:call-template name="complexElementGuiWrapper">
			<xsl:with-param name="title" select="/root/gui/strings/gmiTabQua"/>
			<xsl:with-param name="id"
				select="generate-id(/root/gui/strings/gmiTabQua)"/>
			<xsl:with-param name="content">
	...
	<xsl:template name="gmiTabAcc">
		...
		<xsl:call-template name="complexElementGuiWrapper">
			<xsl:with-param name="title" select="/root/gui/strings/gmiTabAcc"/>
			<xsl:with-param name="id"
				select="generate-id(/root/gui/strings/gmiTabAcc)"/>
			<xsl:with-param name="content">
				...
				<xsl:apply-templates mode="elementEP"
					select="gmd:identificationInfo/*/geonet:child[string(@name)='resourceConstraints']
					">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit" select="$edit"/>
					<xsl:with-param name="force" select="true()"/>
	...
	<!-- gmiTabMd: cares about metadata information -->
	<xsl:template name="gmiTabMd">
		...
		<!-- Debug start -->
		<xsl:message> #### Template gmiTabMd mode ### </xsl:message>
		
		<xsl:call-template name="complexElementGuiWrapper">
			<xsl:with-param name="title" select="/root/gui/strings/gmiTabMd"/>
			<xsl:with-param name="id"
				select="generate-id(/root/gui/strings/gmiTabMd)"/>
			<xsl:with-param name="content">
